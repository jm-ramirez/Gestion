...
Public Class FormCbteVentaKiosco
    ...
    Private Sub InicializarFormulario()
        ...
        Select Case Me.TipoCbte
            ...
            Case TipoEmisionCbte.PRESUPUESTO : Me.Text = "Emisión de Nota de Pedido"
    ...
    'Private Sub FormCbteVentaKiosco_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
    '    If Me.TipoCbte = TipoEmisionCbte.FISCAL Then
    '        Me.CtlCbteDetalle.FocoArticulo()
    '    Else
    ...
    Private Sub FormCbteVentaKiosco_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub InicializarComboPuntosDeVenta()
        ...
        With Me.ComboBoxPtoVta
            ...
            If Me.TipoCbte = TipoEmisionCbte.ELECTRONICO Then
                ...
                If My.Settings.PuntoVtaMayorista <> 0 Then

                    puntos.Add(repositorio.GetByPtoVta(My.Settings.PuntoVtaMayorista))

                    .DataSource = puntos
                Else
                    .DataSource = repositorio.GetPtosElectronicos
                End If

            ElseIf Me.TipoCbte = TipoEmisionCbte.FISCAL Then

                If My.Settings.PuntoVtaMinorista <> 0 Then
                    puntos.Add(repositorio.GetByPtoVta(My.Settings.PuntoVtaMinorista))
                    .DataSource = puntos
                Else
                    .DataSource = repositorio.GetPtosFiscales
                End If

            ElseIf Me.TipoCbte = TipoEmisionCbte.PRESUPUESTO Then
                .DataSource = repositorio.GetPtosPresupuestos
            ElseIf Me.TipoCbte = TipoEmisionCbte.PREIMPRESO Then
                .DataSource = repositorio.GetPtosManuales
            End If
    ...
    Private Function UltimoCbteReciboAutorizado(cbtetipo As UInt32) As Int32
    ...
    '        Case NOTA_CREDITO_A, NOTA_CREDITO_B, NOTA_CREDITO_C, NOTA_DEBITO_A, NOTA_DEBITO_B, NOTA_DEBITO_C
        ...
        Select Case e.KeyCode
            ...
            Case Keys.Enter
                ...
                If Me.ActiveControl.Name <> "CtlCbteDetalle" Then
                    Me.TextBoxDescuentoGral.Focus() ' SendKeys.Send(vbTab)
                End If
        End Select
    End Sub
    ...
    Private Function CrearComprobante() As Entidades.CbteCabecera
        ...
        Try
            ...
            With cbte
                ...
                .Vendedor = 1 'Convert.ToUInt32(Me.ComboBoxVendedor.SelectedValue)
                .Razonsocial = Me.TextBoxNombre.Text
                .Domicilio = Me.TextBoxDomicilio.Text

                'If Me.ComboBoxRemitos.SelectedItem IsNot Nothing Then
                '    Select DirectCast(Me.ComboBoxTipoCbte.SelectedItem, Entidades.Tipocomprobante).Codigo
                '        Case FACTURA_A, TIQUE_FACTURA_A, FACTURA_B, TIQUE_FACTURA_B, FACTURA_C, TIQUE_FACTURA_C, FACTURA_M
    ...
    Private Function CrearComprobanteRecibo(cbteTipo As UInt32, cbteAntRef As UInt32, cbteAntNro As UInt32, cbteAntPtoVta As UInt32, cbteAntTipo As UInt32) As Entidades.CbteCabecera
        Dim cbte As New Entidades.CbteCabecera
        Dim tipoCbte As Entidades.Tipocomprobante
        Dim reponumeracion As New CapaLogica.CbtenumeracionCLog
        ...
        Try
            ...
            'ActualizaNumeracion()
            If Me.TipoCbte = TipoEmisionCbte.PREIMPRESO Then
                cbteNro = reponumeracion.GetByUltimoNroCbte(repocbtepuntoventa.GetByTipo("M").Ptovta, cbteTipo).Numero + 1
            Else 'If Me.TipoCbte = TipoEmisionCbte.PRESUPUESTO Then
                cbteNro = reponumeracion.GetByUltimoNroCbte(repocbtepuntoventa.GetByTipo("X").Ptovta, cbteTipo).Numero + 1
            End If
            ...
            With cbte
                ...
                .Nroremito = ""
                .Cbtefecha = Me.DateTimePickerFecha.Value.ToString("yyyyMMdd")
                .Cbtenumero = cbteNro 'reponumeracion.GetByUltimoNroCbte(ptovta, cbteTipo) + 1 'Convert.ToUInt32(Me.TextBoxNumero.Text)
                .Cbteptovta = Convert.ToUInt32(Me.ComboBoxPtoVta.SelectedValue)
                .Cbtetipo = tipoCbte.Codigo
                .Letra = tipoCbte.Letra
                .Denominacion = tipoCbte.Denominacion
                ...
                concepto = _repositorioConcepto.GetByCodigo("001") 'DirectCast(Me.ComboBoxConceptoBancario.SelectedItem, Entidades.Conceptobancario)
                ...
                fin.Origen = ORIGEN_FINANCIERO_COBRANZA

                .CbteDetalleFinanciero.Add(fin)

                'AgregarCtaPropia(fin)
    ...
    Private Sub AutorizarCbte(ByVal cbte As Entidades.CbteCabecera)
        ...
        If DirectCast(Me.ComboBoxPtoVta.SelectedItem, Entidades.Cbtepuntoventa).Tipo = PTO_VTA_MANUAL Or DirectCast(Me.ComboBoxPtoVta.SelectedItem, Entidades.Cbtepuntoventa).Tipo = PTO_VTA_X Then
            mlngCopias = -1
        ...
        Try
            ...
            Select Case tipoPtoVta
                Case PTO_VTA_FISCAL

                    'cbte.Cbtenumero = objWSFEV.EmitirCbteFiscal(cbte)

                    'If cbte.Cbtenumero <> 0 Then
                    '    cbte.Resultado = "A"
                    '    mensajes.AppendLine("Se generó el comprobante nº " & cbte.Cbtenumero)
                    'Else
                    '    cbte.Resultado = "R"
                    '    mensajes.AppendLine("No se generó ningún comprobante")

                    '    If MessageBox.Show("No se generó ningún comprobante. Cancela?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                    '        objWSFEV.CancelarCbteFiscal()
                    '    End If

                    'End If

                Case PTO_VTA_ELECTRONICO

                    'cbte = objWSFEV.AutorizarCbte(cbte)

                    'mensajes.AppendLine("Respuesta del servidor: ")
                    'mensajes.AppendLine("Resultado: " & cbte.Resultado)
                    'mensajes.AppendLine("Reproceso: " & cbte.Reproceso)
                    'mensajes.AppendLine("Cae: " & cbte.Cae)
                    'mensajes.AppendLine("Cae Vto: " & cbte.Fechavtocae)
                    'mensajes.AppendLine("Observaciones: " & vbCrLf & cbte.Observaciones)

                    'If cbte.Eventos.Length > 0 Then
                    '    mensajes.AppendLine("Eventos: " & vbCrLf & cbte.Eventos)
                    'End If

                    'If cbte.Errores.Length > 0 Then
                    '    mensajes.AppendLine("Errores: " & vbCrLf & cbte.Errores)
                    'End If

                    'Me.TextBoxCAE.Text = cbte.Cae
                    'Me.TextBoxCAEVto.Text = cbte.Fechavtocae

                Case Else 'x o preimpreso
            ...
            Else
                ...
                Else
                    ...
                    Select Case Me.TipoCbte
                        Case TipoEmisionCbte.ELECTRONICO, TipoEmisionCbte.PREIMPRESO, TipoEmisionCbte.PRESUPUESTO
                            If Me.TipoCbte = TipoEmisionCbte.ELECTRONICO Then
                                EmisionCbte(cbte.Id)
                            Else
                            ...
                            If ComboBoxFormaPago.SelectedValue = FP_CONTADO And ((Me.TipoCbte = TipoEmisionCbte.PREIMPRESO) Or Me.TipoCbte = (TipoEmisionCbte.PRESUPUESTO)) Then
                                Dim cbte2 As Entidades.CbteCabecera

                                Select Case Me.TipoCbte
                                    Case TipoEmisionCbte.PREIMPRESO
                                        cbte2 = CrearComprobanteRecibo(RECIBO_COBRANZA, cbte.Id, cbte.Cbtenumero, cbte.Cbteptovta, cbte.Cbtetipo)
                                    Case TipoEmisionCbte.PRESUPUESTO
                                        cbte2 = CrearComprobanteRecibo(RECIBO_PRESUPUESTO, cbte.Id, cbte.Cbtenumero, cbte.Cbteptovta, cbte.Cbtetipo)
                                End Select


                                If cbte2 IsNot Nothing Then
                                    AutorizarCbteRecibo(cbte2)
                                End If
                            End If
    ...
    Private Sub AutorizarCbteRecibo(ByVal cbte As Entidades.CbteCabecera)
        Dim mensajes As New StringBuilder
        Dim tipoPtoVta As String = ""
    ...
    Private Sub AutorizandoCbte()
        ...
        If ValidarCbte() Then
            ...
            '    Select Case Me.TipoCbte
            '        Case TipoEmisionCbte.PREIMPRESO
            '            cbte2 = CrearComprobanteRecibo(RECIBO_COBRANZA)
            '        Case TipoEmisionCbte.PRESUPUESTO
            '            cbte2 = CrearComprobanteRecibo(RECIBO_PRESUPUESTO)
            '    End Select
    ...
    Private Function ValidarCbte() As Boolean
        ...
        Select Case DirectCast(Me.ComboBoxTipoCbte.SelectedItem, Entidades.Tipocomprobante).Codigo
            Case DEVOLUCION_PRESUPUESTO, NOTA_CREDITO_A, NOTA_CREDITO_B, NOTA_CREDITO_C, NOTA_DEBITO_A, NOTA_DEBITO_B, NOTA_DEBITO_C, TIQUE_NOTA_CREDITO, TIQUE_NOTA_CREDITO_A, TIQUE_NOTA_CREDITO_B
        ...
        Select Case DirectCast(Me.ComboBoxTipoCbte.SelectedItem, Entidades.Tipocomprobante).Codigo
            Case DEVOLUCION_PRESUPUESTO, NOTA_CREDITO_A, NOTA_CREDITO_B, NOTA_CREDITO_C, TIQUE_NOTA_CREDITO, TIQUE_NOTA_CREDITO_A, TIQUE_NOTA_CREDITO_B
        ...
        If ret.Length = 0 Then
            ...
            'Else
            'SI ES UN PRESUPESTO O PREIMPRESO, NO PREGUNTO SI CONFIRMA PORQUE LO VALIDO CUANDO SELECCIONO SI QUIERE IMPRIMIR EL COMPROBANTE
            If DirectCast(Me.ComboBoxPtoVta.SelectedItem, Entidades.Cbtepuntoventa).Tipo <> PTO_VTA_MANUAL And DirectCast(Me.ComboBoxPtoVta.SelectedItem, Entidades.Cbtepuntoventa).Tipo <> PTO_VTA_X Then
    ...
    'Private Sub ComboBoxVendedor_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
    '    CargaDatosVendedor()
    'End Sub
    ...
    Private Sub CustomKeyDown(ByVal sender As Object, ByVal e As KeyEventArgs)
        If e.KeyCode = Keys.Enter Then
            ...
            '    End If
            'End If
            If sender Is TextBoxDescuentoGral Then
                Me.CtlCbteDetalle.FocoArticulo()
    ...
    'Private Sub ComboBoxRemitos_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
    '    CargarArticulosRemito(sender, e)
    'End Sub

    Private Sub BtnArtRemito_Click(sender As System.Object, e As System.EventArgs)
